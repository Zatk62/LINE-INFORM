// 1. เพิ่มข้อมูล Master Data (ใส่ไว้บนสุดของไฟล์)
const VENDOR_MASTER = {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. เรียกฟังก์ชันเพื่อโหลดรายชื่อ Vendor เข้าสู่ระบบค้นหา
    initVendorSearch();
...
// 3. สร้างฟังก์ชันสำหรับสร้างตัวเลือกการค้นหา
function initVendorSearch() {
    const datalist = document.getElementById('vendor-list');
    if (!datalist) return;

    // ล้างข้อมูลเก่าใน datalist (ถ้ามี)
    datalist.innerHTML = '';

    // วนลูปรายชื่อจาก VENDOR_MASTER เพื่อสร้าง <option>
    // เราจะใช้รูปแบบ "รหัส : ชื่อบริษัท" เพื่อให้ค้นหาได้ทั้งสองอย่าง
    Object.keys(VENDOR_MASTER).forEach(code => {
        const option = document.createElement('option');
        const vendorName = VENDOR_MASTER[code];
        option.value = `${code} | ${vendorName}`; // เก็บทั้งรหัสและชื่อ
...
if (pageId === 'logbook') {
        ...
        // แสดง Skeleton 3 ใบก่อนโหลดจริง
        list.innerHTML = '<div class="skeleton-card"></div>'.repeat(3);
        
        // หน่วงเวลาจำลองการโหลด 500ms เพื่อความ Smooth
        setTimeout(() => {
            renderLogbook();
        }, 500);
    }
...
function togglePresets() {
    ...
    // ถ้าลิ้นชักถูกเปิด ให้ทำการ Render รายการใหม่เสมอเพื่อให้ข้อมูลเป็นปัจจุบัน
...
function renderPresets() {
    ...
    // 1. ถ้ายังไม่มีประวัติเลย ให้แสดงข้อความแนะนำ
    if (logs.length === 0) {
        drawer.innerHTML = '<div style="padding:1rem; font-size:0.75rem; color:#94a3b8; text-align:center;">ยังไม่มีประวัติบันทึก บันทึกข้อมูลก่อนเพื่อสร้างรายการด่วนอัตโนมัติ</div>';
        return;
    }

    // 2. วิเคราะห์หา 5 อันดับที่เกิดบ่อยที่สุด
    const frequencyMap = {};
    logs.forEach(log => {
        // สร้าง Key จาก ชื่อชิ้นงาน + รหัส + ปัญหา เพื่อใช้ในการนับ
        const key = `${log.partName}|${log.partNumber}|${log.problemDesc}`;
        if (!frequencyMap[key]) {
            frequencyMap[key] = { count: 0, data: log };
        }
        frequencyMap[key].count++;
    });

    // แปลงเป็น Array แล้วเรียงลำดับตามจำนวนความถี่ (มาก -> น้อย) และหยิบมาแค่ 5 รายการ
    const topPresets = Object.values(frequencyMap)
        .sort((a, b) => b.count - a.count)
        .slice(0, 5);

    // 3. สร้าง HTML ไปแสดงผล
    drawer.innerHTML = topPresets.map((item) => {
        const p = item.data;
        return `
            <div class="preset-card" onclick="applyFullPreset(${JSON.stringify(p).replace(/"/g, '&quot;')})">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <strong style="color:#facc15;">🔥 พบบ่อย ${item.count} ครั้ง</strong>
                    <span class="sev-tag sev-${p.severity.toLowerCase()}" style="font-size:0.6rem;">${p.severity}</span>
                </div>
                <div style="font-size:0.8rem; font-weight:700; margin:4px 0;">${p.partName}</div>
                <div style="font-size:0.7rem; color:#cbd5e1;">รหัส: ${p.partNumber} | ปัญหา: ${p.problemDesc.substring(0, 30)}...</div>
            </div>
        `;
    }).join('');
}
...
function applyFullPreset(data) {
    ...
    // กรอกข้อมูลจำนวน (ถ้าต้องการให้เป็นค่าเดิมที่เคยบันทึก)
    document.getElementById('totalQty').value = data.totalQty || 0;
    document.getElementById('okQty').value = data.okQty || 0;
    document.getElementById('ngQty').value = data.ngQty || 0;
    
    // กรอกรายละเอียดปัญหาและการจัดการ
    document.getElementById('problemDesc').value = data.problemDesc || '';
    if(document.getElementById('severity')) document.getElementById('severity').value = data.severity;
    if(document.getElementById('disposition')) document.getElementById('disposition').value = data.disposition;
    
    // คำนวณ Rate ใหม่ให้ถูกต้อง
    updateRate();
    
    // ปิดลิ้นชัก Presets
    togglePresets();
    
    // แสดง UI Feedback เล็กน้อย
    const banner = document.querySelector('.banner-form h2');
    const originalText = banner.innerText;
    banner.innerText = '✅ โหลดข้อมูลด่วนเรียบร้อย!';
    banner.style.color = '#34d399';
    setTimeout(() => {
        banner.innerText = originalText;
        banner.style.color = '';
    }, 2000);
}
...
function generateReport() {
    ...
    if (currentFormat === 'chat') {
        txt = `🚨Supports Production Line Report - ${sev.toUpperCase()}]
================================
📦 ชื่อชิ้นงาน: ${pName}
🔢 รหัสชิ้นงาน: ${pNo}
🏢 Supplier: ${vendor}
...
👤 ผู้ตรวจสอบ: ${insp}`;
        ...
        txt = `+-----------------------------------------------------------+
| Supports Production Line Report                          |
+----------------------+------------------------------------+
| Part Name         | ${pName.slice(0, 34).padEnd(34)} |
| Part Number       | ${pNo.padEnd(34)} |
| Vendor            | ${vendor.slice(0, 34).padEnd(34)} |
| Model / Line      | ${model.slice(0, 34).padEnd(34)} |
+----------------------+------------------------------------+
...
+-----------------------------------------------------------+`;
        ...
        txt = `📋 รายงานการตรวจสอบคุณภาพชิ้นงาน
============================================================
ชื่อชิ้นงาน (Part Name): ${pName}
รหัสชิ้นงาน 10 หลัก (Part No.): ${pNo}
ผู้ผลิต (Vendor): ${vendor}
ไลน์ผลิต/รุ่น (Model/Line): ${model}
สรุปยอดตรวจสอบ (Inspection Quantities):
...
📌 ระดับความรุนแรง: ${sev} | การดำเนินการ: ${disp}`;
        ...
        txt = `Supports Production Line Report
...
function copyReport() {
    ...
    // ฟังก์ชันย่อยสำหรับเปลี่ยนสถานะปุ่ม (UI Feedback)
    const updateUI = () => {
    ...
    // --- วิธีที่ 1: ลองใช้ Clipboard API (Modern Way) ---
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(txt)
            ...
            .catch(err => {
                ...
                fallbackCopy(txt, updateUI);
            });
    } else {
        // --- วิธีที่ 2: ใช้ Fallback (Traditional Way) สำหรับ HTTP หรือเมื่อโดนบล็อก ---
        fallbackCopy(txt, updateUI);
    }
}

// ฟังก์ชัน Fallback สำหรับรันบน HTTP หรือสภาพแวดล้อมที่ไม่ปลอดภัย
function fallbackCopy(text, callback) {
    const textArea = document.createElement("textarea");
    textArea.value = text;
    
    // ปรับแต่ง textarea ไม่ให้โผล่มากวนสายตา
    ...
    try {
        ...
        if (successful) {
            ...
            alert('เบราว์เซอร์ปฏิเสธการคัดลอกอัตโนมัติ กรุณาคัดลอกด้วยตนเอง');
        }
    } catch (err) {
        console.error('Fallback copy failed', err);
        alert('เกิดข้อผิดพลาดในการคัดลอก กรุณาคัดลอกด้วยตนเอง');
    }
...
function clearForm() {
    if (confirm('⚠️ ต้องการล้างข้อมูลฟอร์มปัจจุบันทั้งหมดใช่หรือไม่?')) {
        resetFormSilent();
    }
}
...
function runAiSimulation(mode) {
    ...
    const applyBtn = document.getElementById('btnApplyAi'); // อ้างอิงปุ่มใหม่
    
    box.classList.remove('hidden');
    applyBtn.classList.remove('hidden'); // โชว์ปุ่ม Apply เมื่อ AI ทำงานเสร็จ
...
// ฟังก์ชันใหม่สำหรับส่งข้อความกลับไปที่ฟอร์ม
function applyAiResult() {
    const aiText = document.getElementById('aiResultBox').innerText;
    const targetField = document.getElementById('problemDesc');
    
    if (aiText) {
        targetField.value = aiText; // ใส่ข้อความลงในช่อง Description
        closeAiModal(); // ปิด Modal
        
        // เพิ่มเอฟเฟกต์กระพริบที่ช่องเพื่อให้รู้ว่าเปลี่ยนแล้ว (Optional)
        targetField.style.borderColor = '#10b981';
        targetField.focus();
        setTimeout(() => { targetField.style.borderColor = '#475569'; }, 1000);
    }
}
...
// ฟังก์ชันล้างข้อมูลแบบไม่มี Popup ถาม (ใช้ตอนโหลดหน้า)
function resetFormSilent() {
    // 1. ล้างช่องกรอกข้อความทั่วไป (Text Inputs)
    const textFields = ['partName', 'partNumber', 'vendor', 'model', 'problemDesc'];
    textFields.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = '';
    });

    // 2. ล้างช่องจำนวน (Numeric Inputs) 
    // ใส่เป็น '0' เพื่อให้ระบบคำนวณ Rate ได้ถูกต้องทันที
    const qtyFields = ['totalQty', 'okQty', 'ngQty'];
    qtyFields.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = '0';
    });

    // 3. รีเซ็ต Dropdown ทั้งหมดกลับไปที่ตัวเลือกแรก (Index 0)
    // เพิ่ม 'inspector' เข้าไปในรายการนี้เพื่อให้ล้างชื่อผู้ตรวจด้วย
    const selectFields = ['severity', 'disposition', 'inspector'];
    selectFields.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.selectedIndex = 0; 
    });

    // 4. อัปเดตอัตราส่วนงานเสีย (Defect Rate) ให้กลับเป็น 0.00%
    if (typeof updateRate === 'function') {
        updateRate();
    }
    
    // (Optional) ล้าง Badge แจ้งเตือนรหัส 10 หลักถ้ามี
    const badge = document.getElementById('partNoBadge');
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. ล้างค่าในฟอร์มให้ว่างเปล่าทันทีที่โหลดหน้า (Silent Reset)
    resetFormSilent(); 
    
    // 2. โหลดส่วนประกอบอื่นๆ ตามปกติ
    updateRate();
    renderPresets();
    renderLogbook();
    updateBadge();
...
function showToast(message, type = 'success') {
    ...
    // ระบบสั่น (Haptic Feedback) - ทำงานเฉพาะบนมือถือ
...
function handleVendorSearch(query) {
    ...
    // 2. กรองข้อมูลจาก VENDOR_MASTER (ค้นหาได้ทั้ง รหัส และ ชื่อ)
    const matches = Object.entries(VENDOR_MASTER).filter(([code, name]) => {
        return code.toLowerCase().includes(input) || name.toLowerCase().includes(input);
    }).slice(0, 15); // แสดงผลแค่ 15 รายการแรกเพื่อความกระชับ

    // 3. สร้าง HTML รายการผลลัพธ์
    if (matches.length > 0) {
        resultsBox.innerHTML = matches.map(([code, name]) => `
            <div class="result-item" onclick="selectVendor('${code}', '${name}')">
        ...
        resultsBox.innerHTML = '<div style="padding:15px; color:#94a3b8; font-size:0.8rem;">ไม่พบข้อมูลผู้ผลิต</div>';
        resultsBox.classList.remove('hidden');
    }
}

// ฟังก์ชันเมื่อกดเลือกรายการ
function selectVendor(code, name) {
    const vendorInput = document.getElementById('vendor');
    const resultsBox = document.getElementById('vendor-results');
    
    // ใส่ค่าลงใน Input (เลือกรูปแบบที่ต้องการแสดง เช่น รหัส | ชื่อ)
    vendorInput.value = `${code} | ${name}`;
    
    // ซ่อนหน้าต่างผลลัพธ์
    resultsBox.classList.add('hidden');
    
    // UI Feedback
    showToast(`เลือก: ${code}`, 'info');
}
